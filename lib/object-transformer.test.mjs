  ...
  describe('returned function', () => {
    ...
    describe('rules object parameter', () => {
      ...
      it('can parse the value of an input key into a different data type', () => {
        ...
        const rules = {
          key1: (obj, key, value) => (obj[key] = value + ' string'),
          key2: (obj, key, value) => (obj[key] = parseInt(value)),
        };
      ...
      it("can map an input key's value to a different output key", () => {
        ...
        const rules = {
          originalKey: (obj, key, value) => (obj.newKey = parseInt(value)),
        };
      ...
      it('can use a temp object to store values that need to be carried over from one rule to another, and is cleared once transformer is finished', () => {
        ...
        const rules = {
          titles: (obj, key, value) => {
            obj.temp.titles = [...value];
          },
          years: (obj, key, value) => {
            obj.temp.years = [...value];
          },
          mainChars: (obj, key, value) => {
            obj.temp.mainChars = [...value];
          },
          _onFinished: (output) => {
      ...
      it('has an _onFinished function which can use temp object properties before the temp object is removed', () => {
        ...
        const rules = {
          someKey: (obj, key, value) => (obj.temp[key] = value),
          _onFinished: (output) => {
    ...
    describe('options object parameter', () => {
      it('can use an omitRulelessKeys boolean option to leave out any keys that have no rules assigned', () => {
        ...
        const rules = {
          keyToKeep: (obj, key, value) => (obj[key] = value),
        };
      ...
      it('is passed to each rule function, so the user can assign arbitrary options to use in multiple rules', () => {
        ...
        const rules = {
          myKey: (obj, key, value, opts) =>
            (obj[key] = value * opts.multiplier),
        };
