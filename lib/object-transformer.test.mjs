...
describe('object-transformer', () => {
  ...
  describe('returned function', () => {
    ...
    describe('options object parameter', () => {
      ...
      it('can use an omitEmptyStrings boolean option to leave out any values that are empty strings', () => {
        const inputObj = {
        ...
        const rules = {
          keyToKeep: ({ output, key, value }) => (output[key] = value),
          keyToIgnore: ({ output, key, value }) => (output[key] = value),
        };
      ...
      describe('pathSeparator', () => {
        ...
        it('can assign null to pathSeparator to prevent usage of nested object paths', () => {
          const inputObj = {
            my: {
              nested: {
                key: 'my nested value',
          ...
          const rules = {
            my: ({ output, key, value }) => (output[key] = value),
            'my.nested.key': ({ output, key, value }) => (output[key] = value),
            'my.ignored.key': ({ output, key, value }) => (output[key] = value),
          };

          const t = transformer(rules, { pathSeparator: null });
          const outputObj = t(inputObj);
