 ...
 *
 * @param {Object} [rules=defaultRules] - An optional object that contains functions which correspond to keys on the input object and build up the output object. Each rule can take a single parameter of type object that contains the properties output, key, value, options.
 * @param {rule} [rules._onStart=defaultRules._onStart] - A function that runs before all other rules, but after the output._temp object is created.
 * @param {rule} [rules._onFinish=defaultRules._onFinish] - A function that runs after all other rules, but before the output._temp object is deleted.
 *
 * @param {Object} [options=defaultOptions] - An optional options object to change the default options. This gets passed to each function on the rules object.
 * @param {boolean} [options.omitRulelessKeys=defaultOptions.omitRulelessKeys] - Whether keys with no user-defined rule should be copied over to the output or ignored.
 * @param {boolean} [options.omitEmptyStrings=defaultOptions.omitEmptyStrings] - Whether keys with a value equal to an empty string should have their rule run or be ignored.
 * @param {string} [options.pathSeparator=defaultOptions.pathSeparator] - The delimiter to use for nested object paths.
 * @param {boolean} [options.nestedInputKeys=defaultOptions.nestedInputKeys] - Whether the input keys should use the pathSeparator to determine object nesting, or be treated as flat keys - i.e. if false, 'my.nested.key' and 'myKey' are both on the root of the object.
 * @param {boolean} [options.nestedOutputKeys=defaultOptions.nestedOutputKeys] - Whether the output keys should use the pathSeparator to determine object nesting, or be treated as flat keys - i.e. if false, 'my.nested.key' and 'myKey' are both on the root of the object.
 *
 * @returns {(input:Object) => Object} A function that takes an input object, runs the rules that build up the output object, and then returns the output object.
 */
