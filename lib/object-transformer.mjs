export default function transformer(rules = {}, options = {}) {
  ...
  return (inputObj) => {
    ...
    allRules._onStart({ output: outputObj, input: inputObj, options });

    for (const key in inputObj) {
      const value = inputObj[key];
      if (allRules[key])
        allRules[key]({ output: outputObj, key, value, options });
    ...
    allRules._onFinished({ output: outputObj, input: inputObj, options });
